...
namespace Nase
{
    ...
    public class Logger
    {
        ...
        public void FatalException(Exception exception, string message) { LogException(Level.Fatal, message, null, exception); Environment.Exit(1); }
        public void FatalException(Exception exception, string message, params object[] args) { LogException(Level.Fatal, message, args, exception); Environment.Exit(1); }
    }
    ...
    public class FileTarget : Target, IDisposable
    {
        ...
        {
            this._logStream.WriteLine(string.Format("[{0}] [{1}]: {2} :\n[Exception] {3}", level.ToString().PadLeft(5), logger, message, exception));
        }
